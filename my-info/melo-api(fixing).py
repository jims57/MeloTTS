                    ...
                    try:
                        ...
                        for i in range(0, len(segment_audio), samples_per_chunk):
                            ...
                            if audio_format.lower() == "pcm":
                                ...
                                volume_multiplier = 2.0
                                if language == "ZH":
                                    volume_multiplier = 8.47  # 中文增加13dB
                                ...
                                try:
                                    ...
                                    # 使用FFmpeg转换为原始MP3帧（无Xing头、无ID3标签）
                                    process = subprocess.Popen(
                                        [
                                            'ffmpeg',
                                            '-f', 's16le',           # 输入格式：16位小端序PCM
                                            '-ar', str(output_sample_rate),  # 输入采样率
                                            '-ac', '1',              # 单声道
                                            '-i', 'pipe:0',          # 从stdin读取
                                            '-c:a', 'libmp3lame',    # MP3编码器
                                            '-b:a', '128k',          # 128 kbps比特率
                                            '-q:a', '2',             # 质量设置
                                            '-write_xing', '0',      # 关键：不写入Xing头
                                            '-id3v2_version', '0',   # 关键：不写入ID3标签
                                    ...
                                    print(f"[WS-TTS] 🎵 Raw MP3 frames encoded: {len(mp3_data)} bytes")
                                ...
                                if mp3_data:
                                    ...
                                    chunk_processing_time = (time.time() - chunk_start_time) * 1000
                                    print(f"[WS-TTS] 📦 Raw MP3 chunk {chunk_counter} sent: {len(mp3_data)} bytes, time: {chunk_processing_time:.2f}ms")
